...
- **History clicks: single to inspect, double to edit.** A single click on a
  ...
  made, without moving the model. A double-click edits it: the rollback marker
  drops onto that step (the steps after it roll away, so the timeline reads the
  same as the view), its own inputs come on screen, and finishing or Escape
  springs the marker back to the tip. The saved marker is left untouched
  throughout, so a save mid-edit still records the latest. This replaces the
  older behaviour where a single click quietly opened the editor and a
  double-click scrubbed the view. A history click, the transport buttons and the
  marker drag are all inert while an edit or sketch is in progress, so nothing
  gets swapped out from under a running tool.
...
- **An imported file's own colours become materials.** A STEP assembly that was coloured in the CAD system it came from arrives coloured here: each colour becomes a material, named for the colour when the file gives it no name, and the parts wear it. A colour close to something already in the library reuses that material rather than adding a near-identical row, so importing two revisions of an assembly does not leave you with two libraries of similar greys. A subassembly's colour applies to the parts inside it, which is how assemblies are usually coloured; a part the file left alone is left alone here too. The colours were always read, carried in the import, and then dropped on the floor, so a fully coloured assembly used to open as a few thousand identical grey bodies.

- **Materials, and a library to keep them in.** Edit ▸ Materials, or the command palette, opens the library: ten starter materials, an editor for colour, metalness, roughness and opacity, and Apply to whatever bodies are selected. A body also has Material on its right-click menu, in the tree and on the model, and it acts on the whole selection when the body you clicked is part of one. The library is a plain JSON file you can export, hand to somebody, and import again: importing merges by id, so bringing in an edited export updates what you already had instead of unassigning every body wearing something the incoming file happens not to contain. Materials are appearance only, nothing here is sent to the modelling engine and nothing about the geometry changes.

- **Viewport lighting, reflections and background are settings now**, in Preferences. Reflections default to a studio environment, which is what makes a metal look like metal: a physically-based metal is almost entirely reflection, so with nothing to reflect it renders nearly black. Flat is still there and is the clearer way to read a shape. Background can follow the theme or sit on a fixed dark, mid grey or light ground, which is what you want when comparing two finishes. Brightness moves the lights and the reflections together so the two cannot drift apart.

- **Bodies can be filed into folders of your own.** Right-click Bodies and pick New element, or right-click a body (in the tree or in the model) and use Move to, and drag rows into a folder once it exists. Elements nest, they carry the visibility toggle for everything inside them, and a body that is in none of them sits at the top level exactly as it always has. This is aimed at a large import: a file that arrives as three thousand bodies is one list nobody can read, and the structure the file itself carried, where it carried any, is somebody else's idea of how the thing is organised rather than yours. Selecting several bodies and choosing New element files all of them at once, which is the whole gesture for pulling a subassembly out of an import worth sorting. Deleting an element never deletes what was in it: its contents move up to where the folder was. None of this is geometry. Nothing here is sent to the modelling engine, and a document with every element deleted rebuilds byte for byte the same as one that never had any, because sorting a model must not be able to change it.

- **An assistant connected over MCP can read a geometry file in.** A new `doc_import` tool takes a STEP, STL, 3MF, OBJ, BREP or GLB file and puts it in the timeline as a body, so asking one to design a bracket that fits a part now starts with the part rather than with a description of it. Once imported it is ordinary geometry: it can be measured, cut, filleted and modelled against, and `inspect` gives the selectors for its faces and edges. Importing was the one thing the app could do that an assistant could not, since an import feature refers to its geometry by content hash and only the import itself can put the geometry where that hash resolves. The file does not have to be sitting on this computer: an assistant that is holding the file rather than a path to it, an upload, or something in its own sandbox, can send the file itself with the call, and a text format like STEP can go as it is. A host that hands its model a file does not always hand it a path, and asking for one it does not have was enough to stop an import that was otherwise ready to go. It can also send the file compressed, and in pieces. Both matter for a part of any real size: a STEP file is text and gzips to about a tenth, and pieces lift the limit from what fits in one message to as many as it takes, so a part that arrived as an upload is now importable rather than merely nearly. Nothing reaches the document until the last piece does, so a half-sent file never appears in the window as an edit. And when a file is too big even for that, the tool says what to do instead, in the refusal itself: ask for its path on this computer, or open it here with File, Import Mesh, which puts it in the document for the assistant to measure. That last part is the one that matters most. An assistant that decides a reference part cannot be sent does not stop, it models against a simplified stand-in it invented, and the result is wrong in the one way nothing afterwards catches, because every measurement it takes agrees with every other one.

- **A bug report now carries what the renderer actually did.** The report the bug button copies has a new section listing the last stretch of the pipeline that turns a rebuild into what you see: what the geometry engine sent, which bodies were rebuilt and which were reused untouched, when the app and the engine lost sync, and the result of the check that the scene holds exactly the model. Anything that fails that check is kept for the whole session rather than scrolling away, because the gap between something looking wrong and somebody deciding to report it is usually however long they spend trying to work around it first.
...
- **A texture's heightmap can be changed after the fact.** Choosing Heightmap in a committed texture's values now offers the image as well, with the same file picker the tool panel has. Before, that dropdown could switch the pattern to the one that reads a picture and then give you nowhere to name one, so the only way to a different image was to delete the texture and make it again.
...
- **A press/pull whose `operation` disagrees with its distance is named before the build.** On a press/pull `operation` is only a label for the sign of the distance, so an assistant that wrote `"operation": "cut"` with a positive distance got a join with nothing said. The MCP check now names the mismatch and points to `mode`, which the schema now documents as the field that makes a push sweep the face into a prism and combine it.

- **A cut that severs a small piece off a body keeps it, and says so.** A piece under 0.1% of the body it came off was taken for leftover boolean residue and silently thrown away, so pushing a pin channel's conical ceiling up with press/pull, which cut through the thin spike around it, lost the whole tip above the break with nothing said. Only true residue, under a millionth of the body, is dropped now. The timeline chip and the MCP build reply say "the cut split Body1 into 2 pieces", and a press/pull whose offset runs out through a face it does not touch adds "the offset broke through the outside of Body1".
...
- **An assistant connected over MCP now picks up FundaCAD when you open it, instead of only when it was already running.** The connector asked "is FundaCAD open?" once, when it started, and an assistant app starts its connectors when the assistant itself launches, not when you begin a conversation. So the question was asked before you had any reason to open FundaCAD, and answering no meant the assistant worked on a private copy for the rest of that session, however early you opened the app afterwards. It looked like the connector refusing to use the app in front of it. It now asks again while the answer can still change, and switches to your open document when it finds one. Work an assistant has already built on its private copy is left alone, since adopting your document would throw that away, and it says in its log which of the two it is on and where it looked for the app.

- **A body no longer draws shredded and doubled after a rebuild.** When a rebuild arrived in pieces, the previous version of a body was held on screen so there was never a hole where it used to be, and it was meant to be swapped out the moment its replacement landed. If that replacement never arrived, the swap never happened and the old copy was left in the scene with nothing pointing at it, so the finished rebuild drew its own new copy on top. Two versions of one body a hair apart fight over every pixel they share, which is why it looked like a corrupt or badly tessellated mesh: speckled dark facets across the surface, doubled edges, and every hole appearing twice. It was neither. The old copy is now dropped when the rebuild takes over, and the app checks after every rebuild that the scene holds exactly the model and nothing more, so if anything like this happens again it is recorded rather than left to a screenshot.

- **The extrude arrow can be dragged, including when it is inside the part.** Re-opening a committed extrude to change its depth seeded the number into the box and locked it, so that a hand that merely moved across the viewport could not overwrite a saved depth. Nothing could unlock it except retyping, which left an arrow drawn on screen that did nothing at all when pulled. You can now take hold of the arrow and drag it, and doing so takes the number back from the keyboard the way dragging any other handle in the app does. The arrow also draws through solid material now, like every other handle: an extrude that pushes into the part put its own arrow inside the part, so the control vanished exactly when the operation was a cut.
