...
// FundaCAD: a point just off an edge that runs along the surface's own
// boundary sends Newton past the bounds, and the fallback is a global grid
// search per point, seconds per edge on a large B-spline. The foot of such a
// point is on the boundary, so finish there with a 1D Newton along it and
// leave anything that does not settle to the original fallback.
static Standard_Integer BoundaryNewton(GeomAdaptor_Surface& S, const gp_Pnt& P3D,
                                       ...
                                       const gp_Pnt2d& start, gp_Pnt2d& sol)
{
  const Standard_Boolean outU = (U < uf || U > ul) && !S.IsUPeriodic();
  const Standard_Boolean outV = (V < vf || V > vl) && !S.IsVPeriodic();
  if ((U < uf || U > ul) && !outU) return 0;
  if ((V < vf || V > vl) && !outV) return 0;
  U = outU ? (U < uf ? uf : ul) : Max(uf, Min(ul, start.X()));
  V = outV ? (V < vf ? vf : vl) : Max(vf, Min(vl, start.Y()));
  const Standard_Boolean alongV = outU && !outV;
  const Standard_Boolean alongU = outV && !outU;
  gp_Pnt pnt;
  gp_Vec ru, rv, ruu, rvv, ruv;
  Standard_Boolean settled = !(alongU || alongV);
  for (Standard_Integer i = 0; i < 25 && !settled; i++) {
    S.D2(U, V, pnt, ru, rv, ruu, rvv, ruv);
    const gp_Vec rs(pnt, P3D);
    const gp_Vec& d1 = alongV ? rv : ru;
    const gp_Vec& d2 = alongV ? rvv : ruu;
    const Standard_Real g2 = d1 * d1 - rs * d2;
    if (g2 <= 1e-12) return 0;
    const Standard_Real dt = (rs * d1) / g2;
    Standard_Real& t = alongV ? V : U;
    const Standard_Real lo = alongV ? vf : uf, hi = alongV ? vl : ul;
    t += dt;
    if (t < lo || t > hi) {
      t = Max(lo, Min(hi, t));
      settled = Standard_True;
    }
    else if (fabs(dt) <= Max(1e-12, fabs(t) * 10e-16)) {
      settled = Standard_True;
    }
  }
  if (!settled) return 0;
  S.D1(U, V, pnt, ru, rv);
  const gp_Vec rs(pnt, P3D);
  // The clamped direction has to point away from the surface, or the foot is
  // inside after all and only the global search finds it.
  if (outU && (U <= uf ? rs * ru > 1e-12 : rs * ru < -1e-12)) return 0;
  if (outV && (V <= vf ? rs * rv > 1e-12 : rs * rv < -1e-12)) return 0;
  sol.SetCoord(U, V);
  return 1;
}
  ...
  gp_Pnt2d &sol)
{
  ...
  for (Standard_Integer i = 0; i < 25; i++) {
    ...
    if (U < UF || U > UL || V < VF || V > VL)
      return BoundaryNewton(SurfAdapt, P3D, U, V, UF, UL, VF, VL, p2dPrev, sol);
