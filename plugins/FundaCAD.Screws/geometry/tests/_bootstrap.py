...
assert plugin_geometry.generator_for("fastener") is not None, (
    "the fastener generator did not register: "
    f"(broken: {plugin_geometry.broken_plugins()})"
)

SAMPLES = os.path.join(os.path.dirname(os.path.abspath(__file__)), "samples.json")
