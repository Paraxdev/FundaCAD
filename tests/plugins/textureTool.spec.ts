  ...
  describe("committing", () => {
    it("adds a feature bound to the body the faces are on", async () => {
      ...
      panel.commit({
        ...
        profile: "facet", boundaryInset: 0, grime: 0, smooth: 0, projection: "triplanar", seamBlend: 0.5, seamBand: 0.5, amplitude: 1, slopeMin: 0, slopeMax: 180, direction: "out", seed: 1, invert: false,
      });
    ...
    it("writes only the fields the chosen kind reads", async () => {
      ...
      panel.commit({
        ...
        profile: "facet", boundaryInset: 0, grime: 0, smooth: 0, projection: "triplanar", seamBlend: 0.5, seamBand: 0.5, amplitude: 1, slopeMin: 0, slopeMax: 180, direction: "out", seed: 99, invert: false,
      });
  ...
  describe("re-opening a committed texture", () => {
    ...
    it("replaces in place rather than adding a second feature", () => {
      ...
      panel.commit({
        ...
        profile: "round", boundaryInset: 0, grime: 0, smooth: 0, projection: "triplanar", seamBlend: 0.5, seamBand: 0.5, amplitude: 1, slopeMin: 0, slopeMax: 180, direction: "out", seed: 1, invert: false,
      });
      expect(calls.added).toEqual([]);
